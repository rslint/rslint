...
impl Table {
    ...
    pub fn render_top(&mut self) {
        write!(&mut self.renderer, "╒").expect("Failed to write to stdout");
        ...
        for (idx, cell_len) in column_sizes.iter().enumerate() {
            ...
            if idx != column_sizes.len() - 1 {
                ...
                writeln!(&mut self.renderer, "╕").expect("Failed to write to stdout");
            }
        }
    }
    ...
    pub fn render_middle(&mut self, double_lines: bool) {
        ...
        write!(&mut self.renderer, "{}", left).expect("Failed to write to stdout");
        ...
        for (idx, cell_len) in column_sizes.iter().enumerate() {
            ...
            if idx != column_sizes.len() - 1 {
                write!(&mut self.renderer, "{}", intersect).expect("Failed to write to stdout");
            } else {
                writeln!(&mut self.renderer, "{}", right).expect("Failed to write to stdout");
            }
        }
    }
        ...
        for (idx, column) in column_sizes.iter().enumerate() {
            ...
            if let Some(cell) = rows.get(idx) {
                if let Some(color) = cell.color {
                    ...
                    write!(&mut self.renderer, "{}", cell.text).expect("Failed to write to stdout");
    ...
    pub fn render_notes(&mut self) {
        for note in self.notes.iter() {
            ...
            if let Some(color) = note.color {
                ...
                writeln!(&mut self.renderer, "{}", note.text).expect("Failed to write to stdout");
