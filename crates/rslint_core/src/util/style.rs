...
pub fn tokens_have_whitespace(tokens: &[&SyntaxToken], include_linebreaks: bool) -> bool {
    for ws in tokens {
        if parseutil::contains_js_whitespace(ws.text())
            || (include_linebreaks && parseutil::contains_js_linebreak(ws.text()))
        {
            return true;
        }
    }
