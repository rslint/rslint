...
pub fn is_xid_continue(c: char) -> bool {
  XID_CONTINUE_SET.contains(&c)
}

pub fn is_xid_start(c: char) -> bool {
  XID_START_SET.contains(&c)
}

pub static XID_CONTINUE_SET: Lazy<FnvHashSet<char>> = Lazy::new(|| {
  let mut set: FnvHashSet<char> = FnvHashSet::with_capacity_and_hasher(1427, std::default::Default::default());
  set.extend(vec![
...
pub static XID_START_SET: Lazy<FnvHashSet<char>> = Lazy::new(|| {
  let mut set: FnvHashSet<char> = FnvHashSet::with_capacity_and_hasher(1219, std::default::Default::default());
  set.extend(vec![
