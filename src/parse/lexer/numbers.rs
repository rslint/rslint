...
impl<'a> Lexer<'a> {
  ...
  pub fn read_hex_literal(&mut self) -> (Option<Token>, Option<LinterDiagnostic<'a>>) {
    let start = self.cur;
    ...
    match self.peek() {
      ...
      Some(c) if c.to_ascii_lowercase() == 'x' => {
        ...
        match self.peek() {
          ...
          Some(c) if !c.is_identifier_start() => {
            ...
            let err = LinterDiagnostic::new(self.file_id, LinterDiagnosticType::Lexer(MissingHexDigit), false, "Invalid hex literal without digit")
          ...
          Some(_) => {
            self.advance();
            let err = LinterDiagnostic::new(self.file_id, LinterDiagnosticType::Lexer(InvalidHexCharacter), false, "Invalid character in hex literal")
        ...
        loop {
          match self.peek() {
            ...
            Some(c) if !c.is_ascii_hexdigit() => {
              ...
              let err = LinterDiagnostic::new(self.file_id, LinterDiagnosticType::Lexer(InvalidOctalLiteral), false, "Invalid character in hex literal")
      ...
      Some(c) if c.is_ascii_digit() => {
        ...
        let mut err = LinterDiagnostic::new(self.file_id, LinterDiagnosticType::Lexer(InvalidOctalLiteral), false, "Invalid octal literal in ES5 file")
      ...
      Some(c) if c.is_identifier_start() => {
        self.advance();
        let err = LinterDiagnostic::new(self.file_id, LinterDiagnosticType::Lexer(InvalidHexCharacter), false, "Invalid identifier after hex literal")
  ...
  pub fn read_number(&mut self) -> (Option<Token>, Option<LinterDiagnostic<'a>>) {
    let start = self.cur;
    ...
    loop {
      match self.peek() {
        ...
        Some(c) if c.is_identifier_start() => {
          self.advance();
          return (None, Some(LinterDiagnostic::new(self.file_id, LinterDiagnosticType::Lexer(IdentifierStartAfterNumber), false, "Invalid character after numeric literal")
  ...
  pub fn read_decimal(&mut self, start: usize) -> (Option<Token>, Option<LinterDiagnostic<'a>>) {
    loop {
      match self.peek() {
        Some(c) if c.is_ascii_digit() => { self.advance(); },
        ...
        Some(c) if c == '.' => {
          ...
          return (None, Some(LinterDiagnostic::new(self.file_id, LinterDiagnosticType::Lexer(PeriodInFloat), false, "Invalid period in floating point number")
  ...
  fn read_exponent(&mut self) -> Option<LinterDiagnostic<'a>> {
    let next = self.peek();
    ...
    match self.peek() {
      Some(c) if !c.is_ascii_digit() => 
        return Some(LinterDiagnostic::new(self.file_id, LinterDiagnosticType::Lexer(IncompleteExponent), false, "Incomplete number exponent")
    ...
    loop {
      match self.peek() {
        ...
        Some(c) if c == 'e' || c == 'E' => {
          ...
          return Some(LinterDiagnostic::new(self.file_id, LinterDiagnosticType::Lexer(MultipleExponentsInNumber), false, "Invalid nested exponents")
        ...
        Some(c) if c == '.' => {
          self.advance();
          return Some(LinterDiagnostic::new(self.file_id, LinterDiagnosticType::Lexer(DecimalExponent), false, "Invalid decimal exponent")
...
#[cfg(test)]
mod test {
  use crate::parse::lexer::{lexer::Lexer, token::TokenType::{LiteralNumber, InvalidToken}};
  use crate::linter::diagnostic::LinterDiagnosticType;
  ...
  macro_rules! invalid_num_literal {
    ...
    ($source:expr, $expected_err:ident) => {
      ...
      assert_eq!(tok.lexeme.content($source), $source);
      assert_eq!(lexer_res.1.unwrap().error_type, LinterDiagnosticType::Lexer($expected_err));
    }
  }
